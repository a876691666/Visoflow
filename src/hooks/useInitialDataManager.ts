...
export const useInitialDataManager = () => {
  ...
  let prevInitialData: InitialData | undefined
  ...
  const load = async (_initialData: InitialData) => {
    if (!_initialData || prevInitialData === _initialData) return
    ...
    prevInitialData = initialData
    modelStore.set(initialData)
    ...
    if (initialData.fitToView) {
      ...
      const rendererEl = uiStateStore.rendererEl
  ...
  const clear = () => {
    load({ ...INITIAL_DATA, icons: modelStore.icons, colors: modelStore.colors })
